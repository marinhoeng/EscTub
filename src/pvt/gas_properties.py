...
def calculate_Bg(fd: FluidData, P: float, T: float, Z: float | None = None) -> float:
    if Z is None:
...
def calculate_ρ_gas(fd: FluidData, P: float, T: float, Z: float | None = None) -> float:
    if Z is None:
        Z = calculate_Z(fd=fd, P=P, T=T)
...
def calculate_Cg(fd: FluidData, P: float, T: float, Z: float | None = None) -> float:
    ...
    dZ_dP = (Z_up - Z_down) / (2.0 * δ)

    Cg = (1.0 / P) - 1.0 / Z * dZ_dP  # Pa-1
